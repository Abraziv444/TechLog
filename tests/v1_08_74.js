...
(async () => {
  ...
  t('журнал: окружение (версия, телефон, демо, API), поток камеры, два кадра с размером, ролик в очереди, миниатюры, очередь в IndexedDB',
    ...
    && /плиток в документе: 5 \(фото 3: локально 3, с сервера 0; видео 2: локально 2, с сервера 0\) · миниатюры фото 3\/3 · видео 2\/2/.test(res.log)
    && /очередь документа: 5 файл\(ов\) в IndexedDB/.test(res.log) && /документ открыт заново: плиток 5 \(из очереди 5, с сервера 0\)/.test(res.log)
