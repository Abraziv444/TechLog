...
(async () => {
  ...
  t('журнал: окружение (версия, телефон, демо, API), поток камеры, два кадра с размером, ролик в очереди, миниатюры, очередь в IndexedDB',
    ...
    && /камера открылась за \d+ мс: поток \d+×\d+/.test(res.log) && /кадр 1: \d+ мс · (takePhoto|кадр с потока) · \d+ КБ · TL_/.test(res.log) && /кадр 2: /.test(res.log)
    && /фото 1: TL_\S+ · \d+×\d+ · \d+ КБ \((оригинал|пережато)\) · превью \d+ КБ/.test(res.log)
    && /ролик в очереди: TL_\S+\.(mp4|webm) · video\/\S+ · [\d.]+ МБ · длительность \d+ с/.test(res.log)
    && /плиток в документе: 3 \(фото 2, видео 1\) · миниатюры фото 2\/2 · видео 1\/1/.test(res.log)
    && /очередь документа: 3 файл\(ов\) в IndexedDB/.test(res.log) && /документ открыт заново: плиток 3/.test(res.log)
