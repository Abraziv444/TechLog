...
| Ситуация | Файл |
|---|---|
| Новый проект или база любого возраста | `full-install-1_08_39.sql` — один идемпотентный скрипт, добавляет только недостающее |
| База уже на уровне v1.08.37 | `update-to-1_08_39.sql` — короткая дельта (бухгалтерия: роль accountant, учётные поля документов, `acc_settings`, RPC `acc_doc_mark`, RLS чтения) |
| База уже на уровне v1.08.33 | `update-to-1_08_37.sql`, затем `update-to-1_08_39.sql` |
| База старше v1.08.33 | `full-install-1_08_39.sql` — он же и обновляет |

В конце скрипт печатает NOTICE: «схема соответствует v1.08.39 — всё на месте» либо
