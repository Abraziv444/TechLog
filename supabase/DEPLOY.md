...
Актуально для v1.08.26. Клиент (GitHub Pages) и сервер обновляются независимо:
...
| Ситуация | Файл |
|---|---|
| Новый проект или база любого возраста | `full-install-1_08_26.sql` — один идемпотентный скрипт, добавляет только недостающее |
| База уже на уровне v1.08.24 | `update-to-1_08_26.sql` — короткая дельта (склад: возврат и снимок остатков) |
| База уже на уровне v1.08.23 | `update-to-1_08_24.sql`, затем `update-to-1_08_26.sql` |
| База уровня v1.08.17 и старше | `full-install-1_08_26.sql` — он же и обновляет |

В конце скрипт печатает NOTICE: «схема соответствует v1.08.26 — всё на месте» либо
