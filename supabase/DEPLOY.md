...
Актуально для v1.08.97. Клиент (GitHub Pages) и сервер обновляются независимо:
...
| Ситуация | Файл |
|---|---|
| Новый проект или база любого возраста | `full-install-1_08_97.sql` — один идемпотентный скрипт, добавляет только недостающее |
| База уже на уровне v1.08.71 | `update-to-1_08_97.sql` — бухгалтер правит «Организацию (для PDF)»: политики `org_settings_acc_ins/_upd` и триггер `org_settings_acc_guard` (остальные колонки настроек бухгалтеру не изменить). Edge Functions не менялись |
| База уже на уровне v1.08.48 | `update-to-1_08_71.sql` (две колонки org_settings.study_shuffle и media_lock_approved — включает 1.08.70; если 1.08.51 ещё не ставили — `full-install-1_08_97.sql`), затем `update-to-1_08_97.sql`. После 1.08.71 передеплойте media-delete (замок на архивацию апрувнутого документа, FN_VER 1.08.71) |
| База уже на уровне v1.08.39 | `update-to-1_08_48.sql`, затем `update-to-1_08_71.sql`, затем `update-to-1_08_97.sql` |
| База старше v1.08.39 | `full-install-1_08_97.sql` — он же и обновляет |
