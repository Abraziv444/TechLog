-- TechLog · full-install-1_08_70.sql — полный скрипт схемы (v1.08.70)
--   = full-install-1_08_51.sql + org_settings.study_shuffle (перемешивание вариантов ответов, v1.08.70).
...
-- ---------------------------------------------------------------------
-- Самопроверка v1.08.70 (база 1.08.51 + org_settings.study_shuffle)
-- ---------------------------------------------------------------------
do $$
declare miss text := '';
begin
  ...
  if miss <> '' then
    ...
    raise notice 'TechLog: схема соответствует v1.08.70 — всё на месте.';
  end if;
end $$;
